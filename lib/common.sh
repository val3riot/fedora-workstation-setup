...
validate_config() {
  ...
  local -a bool_vars=(
    ...
    INSTALL_BRUNO INSTALL_NVM INSTALL_MINICONDA INSTALL_SDKMAN INSTALL_OH_MY_ZSH
  ...
  local source_name
  local -a source_vars=(
    OH_MY_ZSH_INSTALL_URL SDKMAN_INSTALL_URL NVM_INSTALL_URL MINICONDA_INSTALLER_URL
    DOCKER_REPO_URL DOCKER_DESKTOP_RPM_URL VSCODE_GPG_KEY_URL VSCODE_REPO_BASEURL
    DBEAVER_RPM_URL BRUNO_RELEASES_API_URL JETBRAINS_TOOLBOX_API_URL
  )
